...
Each region returned by `/api/mood` has an `explanation` field. By
default this is a deterministic rule-based sentence (e.g. "Signals
suggest Berlin is reading stressed moderately, likely driven by housing,
energy, and tech layoffs."). You can optionally have an AI provider
rewrite each region's explanation per request.

### 1. Pick a provider

Add one of these blocks to `.env.local`:
...
Default is `AI_EXPLANATION_PROVIDER=none` → no AI calls happen.

### 2. Restart `npm run dev`

`/api/mood` now returns:

- `explanation` text generated by the chosen provider, kept under
  280 characters, with hedged phrasing ("signals suggest…").
- `explanationSource`: `"openai"`, `"gemini"`, `"mixed"`, or
  `"rule-based"`.

### Fallback behavior

The dashboard never breaks because of AI:

- Provider env unset or `none` → rule-based explanations.
- Provider chosen but API key missing → rule-based explanations + a
  warning string in the API response.
- AI call errors / times out / returns invalid output for a region →
  THAT region falls back to rule-based text; the rest still use AI.
- If every region's call fails, `explanationSource` is `"rule-based"`.

### Reminders

- AI keys go in `.env.local`. Never commit them.
- Calls run server-side from the Next.js route only; keys are never
  shipped to the browser.
- Each `/api/mood` call may produce up to 17 small AI calls (one per
  region, concurrency-3 by default). For cost-sensitive deployments
  keep `AI_EXPLANATION_PROVIDER=none` or stick to a cheap model.
